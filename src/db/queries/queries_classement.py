...
def get_classement_general() -> pd.DataFrame:
    ...
    Calcule pour chaque utilisateur son rang, ses points cumulés, le nombre de paris déposés/scorés et son taux de réussite.

    Retourne :
        pd.DataFrame : Triés par points décroissants puis pseudo alphabétique.
...
def get_stats_par_user(user_id: str) -> dict | None:
    ...
    Filtre la vue vue_classement_general sur l'user_id pour extraire les métriques personnelles affichées sur le profil de l'utilisateur.

    Paramètres :
        user_id (str) : UUID de l'utilisateur.

    Retourne :
        dict | None : clés [rang, pseudo, points_total, nb_paris, nb_paris_scores, taux_reussite], ou None si l'utilisateur n'est pas trouvé.
...
def get_historique_points_user(user_id: str) -> pd.DataFrame:
    ...
    Retourne toutes les courses scorées avec le cumul progressif des points, prêt à être affiché dans un graphique d'évolution.
    ...
    Retourne :
        pd.DataFrame : Triés par date croissante. Vide si aucun historique.
